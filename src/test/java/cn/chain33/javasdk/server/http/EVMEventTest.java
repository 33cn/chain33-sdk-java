...
public class EVMEventTest {
    ...
    public void testEvmTransfer() throws Exception {
        ...
        String TestB_Address = "17myhyHT21AkHAQ7e1zDYy9VskhHWF9BwQ";
        String TestA_Address = "1EaVgu8rjmHz1nL43PUg78EfUcqDWNruhB";
        ...
        tx = EvmUtil.callEvmContract(encodeParams, TestB_Address, "deposit", 200000000L, PrivateKey, SignType.SECP256K1, AddressType.BTC_ADDRESS, 0, "", 1000000);
        txhash = client.submitTransaction(tx);
        ...
        //查询个人地址余额
        result = client.callEVMContractReadOnlyFunc(TestB_Address, new Function("getBalance", Arrays.asList(new AddressBTC(Addr)), Collections.singletonList(new TypeReference<Uint256>() {
        })));
        System.out.println("个人地址："+Addr +" 余额："+result.get(0).getValue());


        //step4 从合约里提款
        encodeParams = FunctionEncoder.encode(new Function("withdraw1", Arrays.asList(new AddressBTC(Addr),new Uint256(100000000L)), Collections.emptyList()));
        tx = EvmUtil.callEvmContract(encodeParams, TestB_Address, "withdraw1", 0, PrivateKey, SignType.SECP256K1, AddressType.BTC_ADDRESS, 0, "", 1000000);
        txhash = client.submitTransaction(tx);
        ...
        //step4 从合约里提款
        encodeParams = FunctionEncoder.encode(new Function("withdraw2", Arrays.asList(new AddressBTC(Addr),new Uint256(100000000L)), Collections.emptyList()));
        tx = EvmUtil.callEvmContract(encodeParams, TestB_Address, "withdraw2", 0, PrivateKey, SignType.SECP256K1, AddressType.BTC_ADDRESS, 0, "", 1000000);
        txhash = client.submitTransaction(tx);
        System.out.println("从evm合约地址往个人地址转账交易哈希 = " + txhash);
        Thread.sleep(20000);
        txResult = client.queryTransaction(txhash);
