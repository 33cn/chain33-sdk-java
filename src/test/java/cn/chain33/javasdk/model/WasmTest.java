...
public class WasmTest {
    ...
    public void testWasmAll() throws Exception {
        ...
        AccountInfo accountInfo = account.loadGMAccountLocal("test", "",
                "./authdir/crypto/org1/user1/keystore/8e6675d9ab566931ca967bb0b520e431d89bde2d9567f50d8ec6a4cad46a8955_sk");
        // 加载用户证书
        ...
        String createTx = TransactionUtil.createTxWithCert(accountInfo.getPrivateKey(), execer, create.toByteArray(),
                SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
        ...
        String callTx = TransactionUtil.createTxWithCert(accountInfo.getPrivateKey(), execer, call.toByteArray(),
                SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
        ...
        String transactionHash = TransactionUtil.createTxWithCert(accountInfo.getPrivateKey(), execer,
                update.toByteArray(), SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
        ...
        String oldCallTx = TransactionUtil.createTxWithCert(accountInfo.getPrivateKey(), execer, oldCall.toByteArray(),
                SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
        ...
        String newCallTx = TransactionUtil.createTxWithCert(accountInfo.getPrivateKey(), execer, newCall.toByteArray(),
                SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
    ...
    public void createWasmContract() throws Exception {
        ...
        WasmProtobuf.wasmAction create = WasmUtil.createWasmContract(contractName, codes);
        String transactionHash = TransactionUtil.createTxWithCert(accountInfo.getPrivateKey(), execer,
                create.toByteArray(), SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
        String hash = chain33client.submitTransaction(transactionHash);
        System.out.println("hash:" + hash);
    }
    ...
    public void createWasmContractGrpc() throws Exception {
        ...
        // byte[] reqBytes = builder.build().toByteArray();
        WasmProtobuf.wasmAction create = WasmUtil.createWasmContract(contractName, codes);
        TransactionAllProtobuf.Transaction transaction = TransactionUtil.createTxWithCertProto(
                accountInfo.getPrivateKey(), execer, create.toByteArray(), SignType.SM2, certBytes,
                SM2Util.Default_Uid);
        // 发送交易
    ...
    public void updateWasmContractGrpc() throws Exception {
        ...
        WasmProtobuf.wasmAction update = WasmUtil.updateWasmContract(contractName, codes);
        TransactionAllProtobuf.Transaction transaction = TransactionUtil.createTxWithCertProto(
                accountInfo.getPrivateKey(), execer, update.toByteArray(), SignType.SM2, certBytes,
                SM2Util.Default_Uid);
        // 发送交易
        CommonProtobuf.Reply result = javaGrpcClient.run(o -> o.sendTransaction(transaction));
        System.out.println("txhash:" + "0x" + HexUtil.toHexString(result.getMsg().toByteArray()));
    ...
    public void callWasmContract() throws Exception {
        ...
        String transactionHash = TransactionUtil.createTxWithCert(accountInfo.getPrivateKey(), execer,
                call.toByteArray(), SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
        String hash = chain33client.submitTransaction(transactionHash);
        System.out.println("hash:" + hash);
    }
    ...
    public void callWasmContractGrpc() throws Exception {
        ...
        TransactionAllProtobuf.Transaction transaction = TransactionUtil.createTxWithCertProto(
                accountInfo.getPrivateKey(), execer, call.toByteArray(), SignType.SM2, certBytes, SM2Util.Default_Uid);
        // 发送交易
        CommonProtobuf.Reply result = javaGrpcClient.run(o -> o.sendTransaction(transaction));
        System.out.println("txhash:" + "0x" + HexUtil.toHexString(result.getMsg().toByteArray()));
    }
