...
public class SimpleStoreGrpc {
    ...
    public void writeData() throws IOException {
        ...
        long txHeight = javaGrpcClient.run(o -> o.getLastHeader(CommonProtobuf.ReqNil.newBuilder().build()))
                .getHeight();
        TransactionAllProtobuf.Transaction transaction = TransactionUtil.createTransferTx2(privateKey, contractAddress,
                execer, content.getBytes(), TransactionUtil.DEFAULT_FEE, txHeight);
    ...
    public void getData() {
        ...
        CommonProtobuf.ReqHash request = CommonProtobuf.ReqHash.newBuilder().setHash(ByteString.copyFrom(hashBytes))
                .build();
        TransactionAllProtobuf.TransactionDetail queryTransaction = javaGrpcClient
                .run(o -> o.queryTransaction(request));
    ...
    public void getParaData() {
        ...
        TransactionAllProtobuf.TransactionDetail queryTransaction = javaGrpcClientPara
                .run(o -> o.queryTransaction(request));
        System.out.println("交易所在的区块高度：" + queryTransaction.getHeight());
        System.out.println("区块的打包时间：" + queryTransaction.getBlocktime());
        System.out.println("从哪个用户发出：" + queryTransaction.getFromaddr());
