 ...
 * 
 * 包含积分名称黑名单，积分的预发行，积分发行，积分增发，积分查询等接口 积分发行步骤： 1. 通过当前链的超级管理员来配置自定义积分的黑名单(全局配置：通常情况下只需要执行一次)。 2.
 * 通过当前链的超级管理员来配置自定义积分的审核者(全局配置：通常情况下只需要执行一次)。 3. 通过积分审核者来预发行自定义积分。 4. 通过积分审核者来正式发行自定义积分。
 * 
 * @author fkeit
 */
public class TokenAllianceTest {
    ...
    public void createTokenTransfer() throws IOException {
        ...
        // 执行器名称
        String execer = "token";
        String createTransferTx = TransactionUtil.createTokenTransferTx(fromAddressPriveteKey, to, execer, amount,
                coinToken, note);
